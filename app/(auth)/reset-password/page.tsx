...
export default function ResetPasswordPage() {
  ...
  const requestReset = useAction(api.passwordResetTokens.mutations.requestReset);
  const verifyCode = useAction(api.passwordResetTokens.mutations.verifyCode);
  const verifyAndReset = useAction(api.passwordResetTokens.mutations.verifyAndReset);
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      const result = await verifyAndReset({ email, token, newPassword: password });
